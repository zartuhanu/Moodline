#!/usr/bin/env python3
"""
Build a flat token corpus and per-song feature table from bassline JSONs.

Input tree:
    data/output_basslines/<artist>/*.json   (from extract_basslines.py)
...
# ---------- I/O locations ----------
JSON_ROOT = Path("data/output_basslines")  # root folder with artist subfolders of .json
...
def tempo_bucket(bpm: float) -> str:
    """Coarse tempo bucket token based on BPM."""
...
def vel_bin(v: int, bins: int = 8) -> int:
    """Quantize MIDI velocity 0–127 into N bins."""
    step = 128 / bins
    return max(0, min(bins-1, int(v // step)))

def ts_tokens(delta: int, max_ts: int = 32):
    """Encode a time shift (in ticks) as repeated TS_* tokens capped at max_ts."""
    toks = []
    while delta > 0:
...
def estimate_bars(events, grid_subdiv: int) -> float:
    """Rough bars estimate assuming 4/4 and grid_subdiv ticks per beat."""
...
def tokenize_song(json_path: Path):
    """Convert one bassline JSON into tokens and a small metadata row."""
